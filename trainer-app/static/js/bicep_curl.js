...
function drawKeypoints(vw, vh) {
  ...
  for (let edge of skeleton) {
    ...
    if (start && end) {
      line(mapX(start.x), mapY(start.y), mapX(end.x), mapY(end.y));
    }
  }
...
function checkArm(arm) {
  ...
  if (Shoulder && Elbow && Hand) {
    ...
    document.getElementById("progressbar").style.width =
      100 * (Math.trunc(curlCounter) / total_reps) + "%";
    // return arm[0];
  }
}
let which_arm = "rightArm";
function draw() {
  if (first) {
    console.log("drawing");
    first = false;
  ...
  if (video && total_reps >= curlCounter) {
    ...
    //   if (which_arm == "leftArm") {
        checkArm(rightArmpoints);
    //   } else {
        // which_arm = checkArm(leftArmpoints);
    //   }
      //  checkArm(leftArmpoints);
	  if(curlCounter>total_reps){
		  window.alert("Excercise Complete");
	  }
    }
  }
}
